...
exports.get = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQueryData(setContext + ` CALL sp_get_technician_customer_feedback(); `, [], supportKey, (error, results) => {
                if (error) {
                    console.log(error);
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`CALL sp_create_technician_customer_feedback(?,?,?,?,?,?,?,?)`, [data.ORDER_ID, data.JOB_CARD_ID, data.TECHNICIAN_ID, data.CUSTOMER_ID, data.RATING, data.COMMENTS, data.FEEDBACK_DATE_TIME, data.CLIENT_ID], supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`CALL sp_update_technician_customer_feedback(?,?,?,?,?,?,?,?,?,?)`, [req.body.ID, data.ORDER_ID, data.JOB_CARD_ID, data.TECHNICIAN_ID, data.CUSTOMER_ID, data.RATING, data.COMMENTS, data.FEEDBACK_DATE_TIME, data.CLIENT_ID, systemDate], supportKey, (error, results) => {
                if (error) {
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    console.log(error);
                    res.send({
...
exports.getTechnicianCustomerFeedback = (req, res) => {
    ...
    const setContext = `
        ...
        SET @v_SORT_VALUE = '${sortValue}';
        SET @v_FILTER = '${safeFilter}';
        SET @v_CUSTOMER_MANAGER_ID = ${CUSTOMER_MANAGER_ID || 0};
    `;
    ...
    try {
        ...
        if (IS_FILTER_WRONG == "0") {
            ...
            mm.executeQueryData(
                setContext + ` CALL sp_get_technician_customer_feedback_manager(); `,
                [],
                supportKey,
                (error, results) => {
