...
SET @v_PAGE_INDEX = ${pageIndex || 0};
            ...
            mm.executeQueryData(
                setContext + ` CALL sp_shipRocketOrderDetails_get(); `,
                [],
                supportKey,
                (error, results) => {
...
SET @v_PAGE_INDEX = ${pageIndex || 0};
            ...
            mm.executeQueryData(
                setContext + ` CALL sp_shipRocketOrderDetails_getById(); `,
                [],
                supportKey,
                (error, results) => {
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(
                `CALL sp_shipRocketOrderDetails_create(?,?,?,?,?,?,?)`,
                [
                    data.ORDER_ID,
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(
                `CALL sp_shipRocketOrderDetails_update(?,?,?,?,?,?,?,?)`,
                [
                    req.body.ID,
                    data.ORDER_ID,
                    data.COURIER_COMPANY,
