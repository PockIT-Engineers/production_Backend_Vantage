...
exports.get = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQueryData(
                setContext + `CALL sp_taxDetails_get()`,
                [],
                supportKey,
                (error, results) => {
                    if (error) {
...
exports.getTaxDetails = (req, res) => {
   const supportKey = req.headers['supportkey'];
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQueryData(
                setContext + `CALL sp_taxDetails_getByState()`,
                [],
                supportKey,
                (error, results) => {
                    if (error) {
    ...
    try {
        ...
        mm.executeQueryData(
            ...
            (error, results) => {
                ...
                var ACTION_DETAILS = `User ${userName} has created a new tax details.`;
                var logCategory = "Tax Details";

                let actionLog = {
                    "SOURCE_ID": resultData.TAX_DETAIL_ID,
                ...
                res.status(200).json({
                    ...
                    "TAX_DETAIL_ID": resultData.TAX_DETAIL_ID
                });
            }
        );
    } catch (error) {
    ...
    try {
        ...
        mm.executeQueryData(
            ...
            (error, results) => {
                ...
                // MongoDB logging (keeping as is)
                var ACTION_DETAILS = `User ${userName} has updated details of the tax details`;
                var logCategory = "Tax Details";
