...
exports.get = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQueryData(
                setContext + `CALL sp_taxStateMapping_get()`,
                [],
                supportKey,
                (error, results) => {
                    if (error) {
...
exports.create = (req, res) => {
    ...
    try {
        ...
        mm.executeQueryData(
            `CALL sp_taxStateMapping_create(?,?,?,?,?,?)`,
            [
                data.TAX_ID,
                data.STATE_ID,
                data.IS_ACTIVE,
            ...
            (error, results) => {
                ...
                var ACTION_DETAILS = `${userName} has created tax state mapping`;
...
exports.update = (req, res) => {
    ...
    try {
        ...
        // Using stored procedure
        mm.executeQueryData(
            `CALL sp_taxStateMapping_update(?,?,?,?,?,?,?)`,
            [
                id,
                data.TAX_ID || null,
                data.STATE_ID || null,
            ...
            (error, results) => {
                ...
                // MongoDB logging (keeping as is)
                var ACTION_DETAILS = `User ${userName} has updated tax state mapping.`;
                var logCategory = "Tax State Mapping";
...
exports.addBulk = (req, res) => {
    ...
    if (!TAX_ID || !data || !Array.isArray(data)) {
        return res.status(400).json({
            "code": 400,
    ...
    try {
        ...
        async.eachSeries(data, function iteratorOverElems(roleDetailsItem, inner_callback) {
            ...
            mm.executeDML(
                `CALL sp_taxStateMapping_upsert(?,?,?,?)`,
                [
                    roleDetailsItem.STATE_ID,
                    TAX_ID,
            ...
            if (error) {
                ...
                var ACTION_DETAILS = `User ${userName} has mapped tax to state`;
                var logCategory = "Tax State Mapping";
